 ...
 * Tarek Eldeeb, https://github.com/tarekeldeeb
 */
...
;( function() {
    ...
    Tree.prototype = {
         ...
         */
        positionNode: function(i, containerCenter, treeCenter, treeWidth, treeHeight, deltaX, deltaY, negOffsetX, negOffsetY){
            var self = this;
            var node = this.nodeDB.get(i);
            ...
            else { // initial creation of nodes, set location
        ...
        positionNodes: function() {
            ...
            var ids = [0], newChildren;
            while(ids.length){
                /*
                This loop starts from th root to the last leaf, guaranteeing all parents are already 
                positioned, regardless of their id (order in db)
                */
                newChildren = this.positionNode(ids[0], containerCenter, treeCenter, treeWidth, treeHeight, deltaX, deltaY, negOffsetX, negOffsetY);
                newChildren && ids.push(...newChildren);
                ids.shift();
            }
            this.addUncles();
            return this;
        ...
        addUncles: function (){
            var self = this, i, len, node;

            // loop all the nodes
            for ( i = 0, len = this.nodeDB.db.length; i < len; i++ ) {
                node = this.nodeDB.get(i);
                if ( !node || typeof node === 'string' || node.id === 0 && this.CONFIG.hideRootNode ) {
                    continue;
                }
                var collapsedParent = node.collapsedParent(),
                    hidePoint = null;
                ...
                if (node.id !== 0 && !(node.parent().id === 0 && this.CONFIG.hideRootNode)) {
                    ...
                    if(node.uncles.length > 0 ){
                        ...
                        if ( this.connectionStore[node.id].length > 1 ) {
                            // connector and uncles already exists
                            this.connectionStore[node.id].slice(1).forEach(function(c,cidx){
								var uncle = this.tree.getNodeDb().get(node.uncles[cidx]);
                                pathString = hidePoint?
                                    this.tree.getPointPathString(hidePoint):
                                    this.tree.getPathString(uncle, node, stacked);
								if(uncle.hidden){
									pathString = "_"+pathString;
								}
                                this.tree.animatePath( c, pathString );
                            });
                        } else {
                            //Add connections to uncles
                            for(u in node.uncles){
                                let uncleNode = {};
                                if( typeof node.uncles[u] === 'number'){
                                    uncleNode = this.getNodeDb().getNodeByDataId(node.uncles[u]);
                                } else {
                                    uncleNode  = this.getNodeDb().getNodeByName(node.uncles[u].text.name);
                                }
                                pathString = hidePoint?
                                    this.getPointPathString(hidePoint):
                                    this.getPathString(uncleNode, node, stacked);
                                connLine = this._R.path( pathString );
                                this.connectionStore[node.id].push(connLine);
        ...
        setConnectionToParent: function( treeNode, hidePoint ) {
            ...
            if ( this.connectionStore[treeNode.id] ) {
                ...
                this.connectionStore[treeNode.id].forEach((c) => this.animatePath( c, pathString ));
            }
            else {
                connLine = this._R.path( pathString );
            ...
            treeNode.connector = connLine; //Not used!
            return this;
        },
    ...
    NodeDB.prototype = {
        ...
        reset: function( nodeStructure, tree ) {
            ...
            this.db = [];
            this.maxid = 0;
            this.db[this.getMaxID(nodeStructure)+1] = "PSEUDO SEPERATOR";
            var self = this;
            ...
            function iterateChildren( node, parentId ) {
                ...
                if ( node.children ) {
                    ...
                    if ( node.childrenDropLevel && node.childrenDropLevel > 0 ) {
                        var level = node.childrenDropLevel;
                        while ( level-- ) {
         ...
         * @returns 
         */
        getMaxID: function(nodeStructure) {
            let maxID = 0;
            JSON.stringify(nodeStructure, (_, nestedValue) => {
              if (nestedValue && nestedValue["id"] > maxID) {
                maxID = nestedValue["id"];
              }
              return nestedValue;
            });
            this.maxid = maxID;
         ...
         * @returns 
         */
        updateNodeOrder: function( node ) {
            var parent = this.db[node.parentId];
            if (parent){
                if(!parent.order) {
                    this.updateNodeOrder(parent);
                }
                if (parent.nodeHTMLclass.includes("wahi")){
                    node.order = "1";
                } else if (node.pseudo){
                    node.order = parent.order;
                } else {
                    node.order = parseInt(parent.order) + 1;
                }
            } 
        },
        /**
        ...
        createGeometries: function( tree ) {
            ...
            while ( i-- ) {
                node = this.get( i );
                if(!node || !node.createGeometry) continue;
        ...
        getNodeByName: function (textName){
            return this.db.find((n) => n.text && n.text.name === textName );
        },
        getNodeByDataId: function (data_id){
            return this.db.find((n) => n && n.data_id === data_id );
        },
        /**
         *
         * @param {object} nodeStructure
        ...
        createNode: function( nodeStructure, parentId, tree, stackParentId ) {
            // db looks like   [node.id, node.id, ... , "", Pseudo, Pseudo, ...]
            //Newly added nodes have no _json, pseudo have none.
            var id = (parentId == -1)? 0: nodeStructure._json_id || nodeStructure.id || nodeStructure.data_id || this.db.length;
            var node = new TreeNode( nodeStructure, id, parentId, tree, stackParentId );
            //TELDEEB: Hack!
            if(node.nodeHTMLclass.trim() === "rawi") node.link.href = '#/إسناد/'+ node.text.name.replace(/ /g,"_");
            this.db[id] = node;
    ...
    TreeNode.prototype = {
        ...
        reset: function( nodeStructure, id, parentId, tree, stackParentId ) {
            ...
            this.date = nodeStructure.date || '';
            this.place = nodeStructure.place || '';
            this.contact = nodeStructure.contact || '';
            this.order = nodeStructure.order ||'';
            this.image_file = nodeStructure.image_file || null;
            this.childrenDropLevel = nodeStructure.childrenDropLevel || null;

            this.children = [];
            this.uncles = nodeStructure.uncles || [];
        ...
        hideConnector: function() {
            var oTree = this.Tree();
            var oPath = oTree.connectionStore[this.id][0];
            if ( oPath ) {
                oPath.animate(
                    { 'opacity': 0 },
                    oTree.CONFIG.animation.connectorsSpeed,
    ...
    TreeNode.prototype.buildNodeFromText = function (node) {
        ...
        // Node Order in Tree
        if (!node.classList.contains("wahi")) {
            order = document.createElement('div');
            order.className = 'node-order';
    ...
    TreeNode.prototype.createGeometry = function( tree ) {
        ...
        /////////// CREATE NODE //////////////
        var node = document.createElement( this.link.href? 'a': 'div' );
    ...
    var JSONconfig = {
        ...
        findChildren: function(nodes) {
            ...
            while(parents.length) {
                ...
                for(;i<len;i++) {
                    var node = nodes[i];
                    if(node.hasOwnProperty('parent') &&( (node.parent._json_id === parentId) || (node.parent == parent.id) )) { // skip config and root nodes
                        var inc_id = this.getID();
                        node._json_id = node.id || inc_id;
    ...
    var Treant = function( jsonConfig, callback, jQuery ) {
        ...
        window.tree = this.tree; //TODO: REMOVE THIS LINE.
    };
