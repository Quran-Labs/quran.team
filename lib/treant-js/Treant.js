                    ...
                    if(node.uncles.length > 0 ){
                        ...
                        if ( this.connectionStore[node.id].length > 1 ) {
                            ...
                            this.connectionStore[node.id].slice(1).forEach(function(c,cidx){
                                let uncleNode = {};
                                if( typeof node.uncles[cidx] === 'number'){
                                    uncleNode = this.tree.getNodeDb().getNodeByDataId(node.uncles[cidx]);
                                } else {
                                    uncleNode  = this.tree.getNodeDb().getNodeByName(node.uncles[cidx].text.name);
                                }
                                pathString = hidePoint?
                                    this.tree.getPointPathString(hidePoint):
                                    this.tree.getPathString(uncleNode, node, stacked);
